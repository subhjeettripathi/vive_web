...
export class ShowsComponent implements OnInit {
  ...
  changeVideoAndBanner(v111: any, v222: any) {
    ...
    this.vid.src({
      ...
      type: this.vid.cache_.source.type,
    });
    ...
    //   lockButton: false
    // }
    //  this.vid1.touchOverlay(options)
    ...
    this.vid1.overlay({
      overlays: [
        {
          start: "playing",
          content: this.gettitle.split('/').join(' ') + ' ' + 'Trailer',
          align: "center",
        },
      ],
    });
        ...
        if (selectedLayout !== undefined) {
          ...
          category.cat_cntn.forEach((cat: any) => {
            ...
            if (cat.is_group == 1 && cat.groupInfo != null) {
              ...
              } else if (category.category_type == "shows") {
                if (cat.groupInfo != null && cat.groupInfo.global_thumb != null && cat.groupInfo.global_thumb.length > 0) {
                  cat.groupInfo.global_thumb.forEach((thumb: any) => {
                    if (thumb != null) {
                      if (thumb.layout == "square") {
                        thumb.layout = "circle";
      ...
      // setTimeout(() => {
        ...
        if (id == 0) {
          ...
          setTimeout(() => {
            ...
            this.isGroupGet = $(`#${this.idd1}`).find("#isGroupGet")[0].className;
  ...
  beforeChange(_e: any, id: any) {
    ...
    if (id == 0) {
      ...
      this.isGroupGet = $(`#${this.idd}`).find("#isGroupGet")[0].className;
      $(".iconlayout").hide();
      ...
      if (this.idd) {
        ...
        $(`#${this.idd}`).find(".iconlayout").show();
        if (this.mainData[0]['auto_play'] == 0 && this.isSubsInfo == 1) {
          setTimeout(() => {
            $(`#${this.idd}`).find(".iconage").show();
          }, 100);
      ...
      setTimeout(() => {
        $("#ageRestrict")
          .appendTo($(`#shows${this.idd.slice(9)}`));
      }, 1000);
